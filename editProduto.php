<?php
    ...
    if ($_POST) {
        ...
        if ($_FILES["foto"]["error"] == 0 ) {
            ...
            $nomeFoto = $prod_selecionado['foto'];
        }
...
<body>
    <div class="container">
        <div class="col-6 offset-3 bg-white p-4">
        <div class="breadCrumbs py-4">
            <a href="logout.php" class="text-danger">Logout </a>  |   <a href="indexProdutos.php">Lista de Produtos</a> > <a href="showProduto.php?id=<?= $ID ?>">Produto </a> > <b>Editar</b>
        </div>
            <form method="POST" enctype="multipart/form-data" class="formEdit">     
                ...
                <div>
                    ...
                    <input type="file" name="foto" class="form-control <?php if (!$ok_foto) { ?> is-invalid <?php } ?>" placeholder="Selecione o arquivo" value="<?= $prod_selecionado['foto'] ?>">
                   
                </div>

                <input type="hidden" name="id" value="<?=$id?>">
                <button class="btn btn-primary col-2  offset-10 right mt-3" type="submit">Salvar</button>
            </form>
        </div>
    </div>
